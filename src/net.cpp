...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Amsterex " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    	{"node", "ame1.amsterex.info"},
		{"node2", "ame2.amsterex.info"},
		{"node3", "ame3.amsterex.info"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("Amsterex-dnsseed");
...
unsigned int pnSeed[] =
{        
//0x3F46EB6D, 0xF45287C2, 0x45E6D150, 0x97E9D150,                       
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Amsterex is probably already running."), addrBind.ToString().c_str());
        else
